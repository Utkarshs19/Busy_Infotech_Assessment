...
function seed() {
  ...
  const managerId = insertUser.run('manager@demo.com', hash('password123'), 'Utkarsh (Manager)', 'manager').lastInsertRowid;
  ...
  // Prices are stored in paise (price_cents column), so ₹120.00 = 12000.
  const insertMenu = db.prepare('INSERT INTO menu_items (name, price_cents, available) VALUES (?, ?, ?)');
  const menu = {
    choleBhature: insertMenu.run('Chole Bhature', 12000, 1).lastInsertRowid,
    pavBhaji: insertMenu.run('Pav Bhaji', 12500, 1).lastInsertRowid,
    samosa: insertMenu.run('Samosa', 2500, 1).lastInsertRowid,
    dosa: insertMenu.run('Masala Dosa', 12500, 1).lastInsertRowid,
    paneerTikka: insertMenu.run('Paneer Tikka', 18000, 1).lastInsertRowid,
    biryani: insertMenu.run('Chicken Biryani', 22000, 1).lastInsertRowid,
    lemonade: insertMenu.run('Fresh Lemonade', 4000, 1).lastInsertRowid,
    chai: insertMenu.run('Masala Chai', 3000, 0).lastInsertRowid, // currently unavailable
    coffee: insertMenu.run('Filter Coffee', 7000, 1).lastInsertRowid,
    gulabJamun: insertMenu.run('Gulab Jamun (2 pc)', 6000, 1).lastInsertRowid,
  };
  ...
  function addLine(orderId, itemKey, qty, instructions) {
    ...
    if (!item) throw new Error(`Unknown menu item key: ${itemKey}`);
  ...
  addLine(o, 'biryani', 1);
  addLine(o, 'pavBhaji', 1, 'Less spicy, allergy to chilli');
  ...
  const lid = addLine(o, 'gulabJamun', 2);
  addLine(o, 'lemonade', 1);
  db.prepare("UPDATE order_lines SET status = 'Void', void_reason = ? WHERE id = ?").run('Kitchen ran out of gulab jamun', lid);
  insertEvent.run(o, 'line_voided', null, null, lid, 'Kitchen ran out of gulab jamun', null, managerId, minsAgo(10));
